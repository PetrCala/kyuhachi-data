...
Business hours publish as raw text only (`businessHours.raw`); no structured parse. Scope: parse single-window cases to a structured `WeeklySchedule`, fall back to `raw`. Dependency: the `WeeklySchedule` shape lives in the app repo's shared types. Risk: Japanese hours text is highly irregular — keep the raw fallback dominant. Lower urgency: the app reads `raw` fine today.
