...
Remaining: (1) **wire it into the publish path** — `businessHours.schedule` is already preserved by `apply.py` but never *written*; the publisher should set it from `parsed_hours_doc()` (do after `feat/catalog-pipeline` lands, since it touches `apply.py`). (2) **app side** (separate repo): the current `WeeklySchedule` can't hold holiday-exception notes, split windows, or Nth-weekday closures — those survive only in `raw`; richer rendering would need a type extension. Risk: Japanese hours text is highly irregular — keep the raw fallback dominant. Lower urgency: the app reads `raw` fine today.
